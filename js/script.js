...
function toTitleCase(str) {
  ...
  function smartCapitalize(word) {
    ...
    // Preserve existing acronyms or mixed-case words
    if (
      /[A-Z]/.test(word.slice(1)) || // mixed/internal caps
    ...
    word = word.replace(
      /([a-z])(\d)([a-z])/g,
      (_, a, b, c) =>
        a.toUpperCase() + b + c.toUpperCase()
    );

    // Capitalize first actual letter
    return word.replace(
      /^[^a-zA-Z]*([a-z])/,
      (match, c) =>
...
// close nav when clicking outside #nav-page
$(document).click(function (e) {

  // if nav is not open, do nothing
  if (!$("html").hasClass("nav-page--open")) return;

  // ignore clicks inside nav page
  if ($(e.target).closest("#nav-page").length) return;

  // ignore clicks on menu button
  if ($(e.target).closest(".nav-icon").length) return;

  // close nav
  $("html").removeClass("nav-page--open");
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (scrollIndicator) {
    ...
    scrollIndicator.addEventListener("click", () => {
      ...
      lenis.scrollTo(window.innerHeight - 60, {
        duration: 1.5
      });
...
// ----------------------------------------------
// SCROLL DOWN BUTTON WITH AN ARROW THAT BOUNCES
// ----------------------------------------------
